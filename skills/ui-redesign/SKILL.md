...
name: ui-redesign
description: >-
  任意项目前端页面/模块的"视觉重做"编排，配合 Claude Design（claude.ai/design）：
  AI 负责备料与落地，Claude Design 由用户本人在浏览器操作。Use when 用户说
  "重新设计/重做这个页面/redesign/换个设计/用 Claude Design 改 XX 页/视觉翻新"
  某个前端页面或模块时。
user_invocable: true
version: "2.2.0"
---
...
本机安装态是 **sync-skills 直连软链**（`~/.claude/skills/ui-redesign` → 仓库工作树），
**改源即生效，无需任何重装**。任何对本 skill 的修改(含 Phase D 自进化回写)走:
```
# 在仓库工作树编辑 skills/ui-redesign/**
git add skills/ui-redesign && git commit -m "..." && git push   # 走 PR 落仓库
```
⚠️ **禁止 `npx skills add --all`/`--skill` 重装**——会把直连软链覆盖为快照拷贝,改源失效
(用 `sync-skills doctor` 检测、`link` 修回);npx 仅用于发布给别人。
查重不新建——遵循 `crystallize-skill` 流水线。
