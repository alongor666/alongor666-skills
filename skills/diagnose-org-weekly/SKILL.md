...
纯三级机构）计划达成率已准确可用；以下待补缺（数据字典与机构映射见 `chexian-report-shell/data/README.md`）：

| 限制 | 现状影响 | 补缺方法 | 前置条件 |
|---|---|---|---|
| 渠道类计划未统计 | 分公司整体达成率偏高约 10-15%（车商/经代/金融同业/重客+"其他"实际保费计入分子、不进分母） | 理清渠道与三级机构重复计算规则后，补渠道类 plan_vehicle 进 data/sx_plan_2026.parquet | 拿到不重复的渠道类年计划 |
| 团队/业务员层计划达成率空 | 团队段 plan_completion 为 None（plan 表仅机构层 organization，无 salesman 拆分） | 补 level='salesman' 的 plan_vehicle | 拿到业务员级年计划 |
| 交叉销售（驾意险）达成率空 | cross_sell_completion 为 None（无 plan_personal） | 补 plan_personal 列 | 拿到驾意险分机构计划 |
| sx_plan_2026 每年更新 | 仅含 2026 年计划 | 每年初重建该 parquet（命令见 data/README.md） | 每年初 |
