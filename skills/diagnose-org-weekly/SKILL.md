...
- **v1.20.1 (2026-06-11)**：cli.py 的 SHELL_ROOT 引导解析对齐 ADR-001 三级优先级——补 `$CLAUDE_SKILLS_DIR` 显式覆盖（最高优先）与 plugins/.agents 安装根兜底，`Path.home()` 病态环境防护；隔离目录 + 环境变量活体实测，仓内常规调用零回归
- **v1.20 (2026-05-29)**：① `--level branch` 分公司层（聚合全部三级机构 + 团队 Top20 + 三级机构维度
  ...
  验证：分公司合计与 DuckDB 直查一致、Top20 团队=20、三级机构=14、同期保单组求和自洽、
  团队下钻键 160=20×8、三级机构层无回归（114 下钻键不变）。
...
SX（山西）2026-06 新接入，计划/交叉销售数据尚不完整。SC（四川）不受影响。机构层（10 个
纯三级机构）计划达成率已准确可用；以下待补缺。SX 计划数据由 **chexian-api ETL** 产出
（`数据管理/warehouse/dim/generate_dim_tables.py --branch-code SX`，源 csv `数据管理/存量数据/sx_plan_<year>_by_org.csv`，
本地 gitignored），技能经 `plan_parquet_for_branch` 读 `validation/SX/dim/plan/latest.parquet`。

| 限制 | 现状影响 | 补缺方法 | 前置条件 |
|---|---|---|---|
| 渠道类计划未统计 | 分公司整体达成率偏高约 10-15%（车商/经代/金融同业/重客+"其他"实际保费计入分子、不进分母） | 理清渠道与三级机构重复计算规则后，补渠道类 plan_vehicle 进 chexian-api `存量数据/sx_plan_<year>_by_org.csv` + 重跑 `generate_dim_tables.py --branch-code SX` | 拿到不重复的渠道类年计划 |
| 团队/业务员层计划达成率空 | 团队段 plan_completion 为 None（plan 表仅机构层 organization，无 salesman 拆分） | 扩 ETL 接业务员级计划源，产 level='salesman' 的 plan_vehicle | 拿到业务员级年计划 |
| 交叉销售（驾意险）达成率空 | cross_sell_completion 为 None（无 plan_personal） | 补 plan_personal 列 | 拿到驾意险分机构计划 |
| SX 计划每年更新 | 仅含 2026 年计划 | 更新 chexian-api `存量数据/sx_plan_<year>_by_org.csv` + 重跑 ETL | 每年初 |

口径校验：SX 起保口径 YTD 8961 万 ≈ 经营快报累计 8997 万（差 0.4%，已对齐）。
