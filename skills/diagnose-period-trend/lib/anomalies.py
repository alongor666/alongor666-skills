"""跨维度 Top 异常排名 — V1 驾驶舱 Hero 红条 / V3 周报 TL;DR 引用 / V4 超表
   ?focus= 跳转锚点的共享数据契约。

设计原则：
  - 亮灯统一从 chexian-report-shell/lib/alerts.py:light() 取，禁止本模块硬编码颜色阈值
  ...
  - spark6：按 PERIOD_KEYS 顺序（36m/24m/yoy/12m/6m/ytd）返回 6 期值，供 sparkline
...
# ===== 时间窗顺序（与 periods.PERIOD_KEYS 同步，但只看 label 不依赖 import） =====
...
def build_drilldown_data(df: pd.DataFrame) -> dict:
    ...
    """
    light_fn = dhr_lib.light  # 不直接用顶层 light，避免循环引用
        ...
        for sec2_id, _ in DRILL_SEC_LIST:
            ...
            for raw_v, _ in prim_vals:
                ...
                for sec2_val, grp in cross.groupby(sec2_f):
                    ...
                    sev_cls, _ = (light_fn("variable_cost_ratio_pct", vcr_ytd, n_ytd)
