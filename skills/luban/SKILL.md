...
name: luban
description: |
  鲁班（Luban）——Skill打磨工坊。用验料、访行、过尺、慢刨、回炉五个工匠动作，把"能用的Skill"打磨成能被理解、安装、传播、验证、持续进化的公共资产，产出《Skill打磨报告》、可直接替换的改写片段和"出师证书"结果卡。
  当用户想升级、优化、打磨、产品化、审计、发布自己开发的Skill，或丢来Skill目录/GitHub链接/SKILL.md说"帮我看看怎么改"时使用。
  触发词：让鲁班看看这个skill、班门打磨、打磨/升级/优化我的skill、skill体检、skill审计、对标同类skill、为什么我的skill没人装、这个skill怎么发布到GitHub/ClawHub、改进SKILL.md。
  不用于从零创建新Skill（用skill-creator）、普通代码review（用code-review）或与Skill资产无关的提示语改写。
user_invocable: true
version: "1.2.0"
---
...
各步骤的输出格式统一收在 [references/report-templates.md](references/report-templates.md)（按报告13节编号）；README/Showcase结构与Skill类型适配收在 [references/polish-guides.md](references/polish-guides.md)。下文每步只写"怎么干、什么红线"，要模板时去对应小节取。
...
输出必须简短、结论先行，格式见 [report-templates.md §1](references/report-templates.md)。
...
输出为对标表（每行一个同行，带URL与"可学的手艺/不能照搬的点"），格式见 [report-templates.md §2](references/report-templates.md)。
...
交叉定位的输出（纵向结论 / 横向结论 / 交叉洞察 / 一句话新定位）格式见 [report-templates.md §3](references/report-templates.md)。
...
对当前Skill打分，满分100。三把尺一起量：**结构尺**量它写得清不清楚，**实测尺**量它跑起来灵不灵，**活体尺**量它在真实世界里活得好不好。不要只看格式。九维评分表（维度/权重/得分/证据/短板/优先级）见 [report-templates.md §4](references/report-templates.md)。
...
**差距清单**输出"我们缺什么"，不要泛泛而谈：按 P0（不补就无法公开/无法信任）/ P1（补上明显提升安装率传播率）/ P2（锦上添花）分级，并写明"与同行相比最缺的3件事"和"最有机会打穿的3件事"，格式见 [report-templates.md §5](references/report-templates.md)。

**三个打磨方向**必须给三个，不能只给一个——方案A细修（把现在的Skill做清楚）、方案B精雕（做出同行没有的可见产物）、方案C开套件（从单Skill升级为小型Skill套件），每个方案写明新定位/改动范围/优点/风险/适合条件，并明确推荐一个、给出理由，格式见 [report-templates.md §6](references/report-templates.md)。
...
候选改写方案的输出（本轮只刨什么、改动边界、预期提升、验证方式、文件变更表、**可直接替换的改写片段成品**）格式见 [report-templates.md §7](references/report-templates.md)。
...
公共Skill必须有"摆出来给人看"的意识。README不是说明书，是安装前的销售页 + 安装后的操作入口——首屏要有一句话钩子、可见产物展示、触发方式和安全边界；showcase按"GIF > 截图 > 真实示例输出 > 前后对比图 > 结果卡片"的顺序优先补"看得见"的证明。

README建议结构与Showcase优先级的完整清单见 [polish-guides.md](references/polish-guides.md)。

---

## 第八步：交活——执行计划与打磨报告

- **执行计划**：按"24小时内 / 3天内 / 7天内 / 本轮不做"四档列任务，格式见 [report-templates.md §9](references/report-templates.md)。
- **出师证书**：报告末尾附一张可截图传播的结果卡（作品 / 打磨前后分数 / 一句话新定位 / 绝活 / 下一步），样式见 [report-templates.md §10](references/report-templates.md)。打磨后分数为预估时必须标注"预估"，只有跑过测试prompt实测的分数才能不带标注。
- **最终报告**：13节完整结构见 [report-templates.md 文末](references/report-templates.md)。
...
核心流程不变（验料 → 访行 → 过尺（含活体） → 慢刨 → 验证门 → 回炉），但工具型/方法论型/工作流型/风格型Skill的查验侧重点不同，分型要点见 [polish-guides.md](references/polish-guides.md)。
