...
- **PPT（`deck-16x9.js`）**：键盘 `← →`／`↑ ↓`／`空格`(Shift+空格回退)／`PageUp·Down` 翻页、`Home·End` 跳首末（不绑点击页面左右/滚轮以免误触）；**触摸左右滑动**（手机/平板）；左右**浮动 ‹ › 箭头**（鼠标/触摸都可点，首/末页自动隐藏对应箭头）；底部**序号条**点任意页码一键直达；**缩略图总览内方向键移动所选页、回车/空格进入**；**Esc 两级退出**——在页面上按 `Esc` 进**缩略图总览**（无序号、纯缩略图），在总览里按 `Esc` 退出全屏；也可点序号条的 **`▦`** 开总览、**`⛶`** 进入/退出全屏演示。⚠️ 真·浏览器全屏态下，首个 `Esc` 由浏览器强制退出全屏（JS 拦不住），此时总览改用 `▦` 打开。
