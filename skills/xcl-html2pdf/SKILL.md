...
description: >-
  ...
  导出一页一张、内容不跨页；含标准皮肤（国家地理风 / 经营仪表盘风 / 墨水风三选一）、零依赖验收
  ...
  KPI 看板、dashboard、机构盯盘、墨水风、暗色 PPT、暗色皮肤、暗场投影、双色极简、深色演示、
...
>
> ## ⚑ PPT 皮肤三选一：国家地理风 / 经营仪表盘风 / 墨水风
> PPT 版式下视觉皮肤再三选一（机制层 deck-16x9.css + 翻页脚本不变）：
>
> | 皮肤 | 何时用 | 用哪套 |
> |---|---|---|
...
> | **墨水风** | 暗场投影、发布会调性、对外分享的克制编辑风 | `deck-16x9.css` + `report-skin-ink.css` + `deck-16x9.js` + `skeleton-ink-16x9.html`（自带演示字号，**不叠 skin-16x9.css**；默认整 deck 暗色，5 套主题改 `:root` 一处全换） |
>
> 仪表盘风组件语汇 + 「进度口径≠最终留存」编码铁律见 [`references/dashboard-deck.md`](references/dashboard-deck.md)。诊断报告（机构盯盘 / 续保 / 赔付 / 渠道复盘）默认用仪表盘风。
> 墨水风 5 套主题 / 明暗规则 / 语义色契约 / 与 magazine-web-ppt 的分工边界见 [`references/ink-deck.md`](references/ink-deck.md)（magazine-web-ppt = 在线 WebGL 杂志网页 PPT，不可 driver 验收；墨水风 = 可验收、可导出 PDF、可离线分发的暗色 deck）。
>
...
1. 复制对应套件到工作目录（`*-deck.css` / `report-skin.css` / `deck.js` 等都**勿改**——版面、视觉规范、翻页机制都在里面）：
   ...
   - **PPT · 墨水风**：`assets/skeleton-ink-16x9.html`、`assets/deck-16x9.css`、`assets/report-skin-ink.css`、`assets/deck-16x9.js`（**不要** skin-16x9.css；皮肤自带演示字号）。5 套主题与三件套组件见 `references/ink-deck.md`；语义色只用上下文变量 / 语义类（`.good`/`.press`/`.neut`、`bar-peak`/`bar-rev`、`b-eng`/`b-zero`），**禁止写死 hex**；切主题后先用骨架 P5 样张页验对比度。
...
- **墨水风 · 语义色写死 hex**：墨水风的利好/压力/中性三色由 `.page`（暗）/ `.light`（亮）上下文变量自动换挡；内容里写死 hex 的颜色在 light 页或换主题后对比度必崩。只用 `.good/.press/.neut`、`bar-peak/bar-rev`、`b-eng/b-zero` 等语义类；切主题后先用骨架 P5 样张页一屏验全。详见 `references/ink-deck.md`。
- **墨水风 · 噪声背景禁用 SVG feTurbulence**：背景底噪必须用皮肤内置的预光栅化 PNG data-uri。换成 SVG feTurbulence data-uri 会让 `Page.printToPDF` 光栅化超时，driver 直接报 `CDP timeout: Page.printToPDF`（实测踩坑，且症状与冷启动竞态假 FAIL 极像——先排查背景再怀疑竞态）。
