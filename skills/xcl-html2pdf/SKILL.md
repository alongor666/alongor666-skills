...
>
> 翻页脚本两套不同：**PPT 用 `deck-16x9.js`**（演示导向：← → 翻页 + 底部序号一键直达 + Esc 缩略图总览），**PDF 用 `deck.js`**（← → / 点击左右 / 滚轮），互不影响。
>
...
- **PPT（`deck-16x9.js`）**：`← →` 翻页（**只认方向键**，去掉了点击左右/滚轮以免误触）；底部**序号条**点任意页码一键直达；**Esc**（或序号条右端 ▦）一键切换**全部页缩略图总览**，点缩略图进入该页、Esc 退出。
