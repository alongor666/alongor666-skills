...
description: >
  ...
  提供与内容无关的 A4 版面盒 CSS、横向翻页脚本，和一个零依赖的验收 driver（实测每页填充率/溢出/真实 PDF 页数）。
  触发词：html2pdf、xcl_html2pdf、做成 PDF、印刷级报告、一页一张、横向翻页卡片、A4 卡片、打印成 PDF、report to pdf、build report、verify card。
...
把任意 HTML 报告做成**屏幕横向一屏一页、打印一页一张 A4** 的印刷级文档。机制与内容分离：本 skill 只管「版面盒 + 翻页 + 打印 + 验收」，具体报告的结构/配色/图形由你自己的内容决定。
...
1. 复制三件套到工作目录：`assets/skeleton.html`、`assets/page-deck.css`、`assets/deck.js`（后两个**勿改**——分页与翻页机制就在里面）。
2. 把 `skeleton.html` 里每个 `<section class="page">` 换成你的内容，按需增删页。本报告的视觉风格写在 `<head>` 内联 `<style>`（skeleton 里已给一套可替换的中性风）。
3. 版面规则（铁律）：
